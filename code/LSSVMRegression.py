"""
    以下代码是py包文件，请勿直接运行
    来自Github Dr. Dr. Danny E. P. Vanpoucke的分享
    该作者基于sklearn的api构建了最小二乘法支持向量机的模型

    感谢开源精神！
    并且，本人对把开源代码注释删掉说这是自己写的代码的行为表示不屑
    中国的学术环境需要大家共同建设！

    The following code is a py package file, do not run directly

    Share from Dr. Dr. Danny E. P. Vanpoucke on GitHub

    The author built the model of least square support vector machine based on Sklearn's API



    Thanks to the open source spirit!

    Also, I am dismissive of removing comments from open source code saying it was my own code

    China's academic environment needs to be built by all!
"""
